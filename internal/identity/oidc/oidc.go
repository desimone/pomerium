...
import (
	...
	"github.com/pomerium/pomerium/internal/oidc"
...
var _ identity.Authenticator = &OpenIDProvider{}

// Name identifies the OpenID Connect provider
const Name = "oidc"

// OpenIDProvider provides a standard, OpenID Connect implementation
...
func New(ctx context.Context, o *identity.Options) (*OpenIDProvider, error) {
	var err error
	var p OpenIDProvider
	if o.ProviderURL == "" {
		return nil, identity.ErrMissingProviderURL
	}
	if len(o.Scopes) == 0 {
		o.Scopes = []string{oidc.ScopeOpenID, "profile", "email", "offline_access"}
	}
	p.Provider, err = oidc.NewProvider(ctx, o.ProviderURL)
	if err != nil {
		return nil, fmt.Errorf("identity/oidc: could not connect to %s: %w", o.ProviderName, err)
	}

	p.Verifier = p.Provider.Verifier(&oidc.Config{ClientID: o.ClientID})
...
func (p *OpenIDProvider) GetSignInURL(state string) string {
...
func (p *OpenIDProvider) Authenticate(ctx context.Context, code string) (*sessions.State, error) {
...
func (p *OpenIDProvider) Refresh(ctx context.Context, s *sessions.State) (*sessions.State, error) {
...
func (p *OpenIDProvider) IdentityFromToken(ctx context.Context, t *oauth2.Token) (*oidc.IDToken, error) {
...
func (p *OpenIDProvider) Revoke(ctx context.Context, token *oauth2.Token) error {
	if p.RevocationURL == "" {
		return identity.ErrRevokeNotImplemented
	}
...
// https://openid.net/specs/openid-connect-frontchannel-1_0.html#RPInitiated
func (p *OpenIDProvider) LogOut() (*url.URL, error) {
	if p.EndSessionURL == "" {
		return nil, identity.ErrSignoutNotImplemented
	}
